...
AuthN and AuthZ components used on the beholder ecosystem.

## Project Overview

`beholder` is the central authentication and authorization service for the Beholder ecosystem. It currently focuses on authentication (AuthN) through the `authn` service.
...
### Bruno API Tests & Synchronization

The Bruno collection containing integration tests is located at `authn/tests/Beholder Authentication Service API`. 

You can keep the Bruno requests in sync with changes in the `openapi.yaml` specification using the provided [justfile](file:///home/randrade/code/faber-numeris/beholder/justfile) recipe:

```bash
# Interactively synchronize the Bruno collection with openapi.yaml
just sync-api
```

This recipe uses `@sayedameer/bruno-openapi-sync` via `npx` to perform interactive, Git-style diffs, allowing you to selectively apply specification updates to your Bruno tests without losing custom scripts or assertions.
