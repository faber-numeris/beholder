...
type userService struct {
	repository     outboundport2.Repository
	mailService    outboundport2.Mailer
	hashingService outboundport2.HashingService
}
...
func (s *userService) RegisterUser(ctx context.Context, user *domain2.User, password []byte) (*domain2.User, error) {
...
func (s *userService) GetUserByID(ctx context.Context, id string) (*domain2.User, error) {
...
func (s *userService) GetUserByEmail(ctx context.Context, email string) (*domain2.User, error) {
...
func (s *userService) UpdateUserProfile(ctx context.Context, userID string, req *domain2.User) (*domain2.User, error) {
	...
	if req.Profile != nil {
		if existingUser.Profile == nil {
			existingUser.Profile = &domain2.UserProfile{}
		}
...
func (s *userService) ListUsers(ctx context.Context, params *inboundport.ListUsersParams) ([]*domain2.User, error) {
	slog.Info("Listing users", "params", params)
	repoParams := &outboundport2.ListUsersParams{
		Email:             params.Email,
...
func (s *userService) VerifyPassword(ctx context.Context, email string, password []byte) (*domain2.UserCredentials, error) {
	...
	match, err := s.hashingService.VerifyPassword(ctx, password, user.PasswordHash)
	if err != nil {
		slog.Error("Failed to verify password", "email", email, "error", err)
		return nil, domain2.ErrInvalidCredentials
	}

	if !match {
		slog.Error("Password does not match", "email", email)
		return nil, domain2.ErrInvalidCredentials
	}

	slog.Debug("Password verified successfully", "email", email)
