...
type combinedRepo struct {
	...
	*mocks2.MockUserConfirmationRepository
}

func (c *combinedRepo) Ping() bool {
	return true
}
...
func TestUserService_RegisterUser(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		confRepo := mocks2.NewMockUserConfirmationRepository(t)
		repo := &combinedRepo{userRepo, confRepo}
		mailService := mocks2.NewMockMailer(t)
		hashingService := mocks2.NewMockHashingService(t)
...
func TestUserService_VerifyPassword(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		hashingService.EXPECT().VerifyPassword(ctx, password, passwordHash).Return(true, nil)
